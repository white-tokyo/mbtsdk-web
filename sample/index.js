...
'use strict';var _keys=require('babel-runtime/core-js/object/keys');var _keys2=_interopRequireDefault(_keys);var _iterator=require('babel-runtime/core-js/symbol/iterator');var _iterator2=_interopRequireDefault(_iterator);var _symbol=require('babel-runtime/core-js/symbol');var _symbol2=_interopRequireDefault(_symbol);var _create=require('babel-runtime/core-js/object/create');var _create2=_interopRequireDefault(_create);var _typeof2=require('babel-runtime/helpers/typeof');var _typeof3=_interopRequireDefault(_typeof2);function _interopRequireDefault(obj){return obj&&obj.__esModule?obj:{default:obj};}var commonjsGlobal=typeof window!=='undefined'?window:typeof global!=='undefined'?global:typeof self!=='undefined'?self:{};function interopDefault(ex){return ex&&(typeof ex==='undefined'?'undefined':(0,_typeof3.default)(ex))==='object'&&'default'in ex?ex['default']:ex;}function createCommonjsModule(fn,module){return module={exports:{}},fn(module,module.exports),module.exports;}var rx_all=createCommonjsModule(function(module,exports){// Copyright (c) Microsoft, All rights reserved. See License.txt in the project root for license information.
;(function(undefined){var objectTypes={'function':true,'object':true};function checkGlobal(value){return value&&value.Object===Object?value:null;}var freeExports=objectTypes[typeof exports==='undefined'?'undefined':(0,_typeof3.default)(exports)]&&exports&&!exports.nodeType?exports:null;var freeModule=objectTypes[typeof module==='undefined'?'undefined':(0,_typeof3.default)(module)]&&module&&!module.nodeType?module:null;var freeGlobal=checkGlobal(freeExports&&freeModule&&(typeof commonjsGlobal==='undefined'?'undefined':(0,_typeof3.default)(commonjsGlobal))==='object'&&commonjsGlobal);var freeSelf=checkGlobal(objectTypes[typeof self==='undefined'?'undefined':(0,_typeof3.default)(self)]&&self);var freeWindow=checkGlobal(objectTypes[typeof window==='undefined'?'undefined':(0,_typeof3.default)(window)]&&window);var moduleExports=freeModule&&freeModule.exports===freeExports?freeExports:null;var thisGlobal=checkGlobal(objectTypes[(0,_typeof3.default)(this)]&&this);var root=freeGlobal||freeWindow!==(thisGlobal&&thisGlobal.window)&&freeWindow||freeSelf||thisGlobal||Function('return this')();var Rx={internals:{},config:{Promise:root.Promise},helpers:{}};// Defaults
...
if(typeof event.data==='string'&&event.data.substring(0,MSG_PREFIX.length)===MSG_PREFIX){runTask(event.data.substring(MSG_PREFIX.length));}};root.addEventListener('message',onGlobalPostMessage,false);scheduleMethod=function scheduleMethod(action){var id=nextHandle++;tasksByHandle[id]=action;root.postMessage(MSG_PREFIX+id,'*');return id;};}else if(!!root.MessageChannel){var channel=new root.MessageChannel();channel.port1.onmessage=function(e){runTask(e.data);};scheduleMethod=function scheduleMethod(action){var id=nextHandle++;tasksByHandle[id]=action;channel.port2.postMessage(id);return id;};}else if('document'in root&&'onreadystatechange'in root.document.createElement('script')){scheduleMethod=function scheduleMethod(action){var scriptElement=root.document.createElement('script');var id=nextHandle++;tasksByHandle[id]=action;scriptElement.onreadystatechange=function(){runTask(id);scriptElement.onreadystatechange=null;scriptElement.parentNode.removeChild(scriptElement);scriptElement=null;};root.document.documentElement.appendChild(scriptElement);return id;};}else{scheduleMethod=function scheduleMethod(action){var id=nextHandle++;tasksByHandle[id]=action;localSetTimeout(function(){runTask(id);},0);return id;};}})();/**
   ...
   */var Notification=Rx.Notification=function(){function Notification(){}Notification.prototype._accept=function(onNext,onError,onCompleted){throw new NotImplementedError();};Notification.prototype._acceptObserver=function(onNext,onError,onCompleted){throw new NotImplementedError();};/**
     ...
     */Notification.prototype.accept=function(observerOrOnNext,onError,onCompleted){return observerOrOnNext&&(typeof observerOrOnNext==='undefined'?'undefined':(0,_typeof3.default)(observerOrOnNext))==='object'?this._acceptObserver(observerOrOnNext):this._accept(observerOrOnNext,onError,onCompleted);};/**
...
AbstractObserver.prototype.next=notImplemented;AbstractObserver.prototype.error=notImplemented;AbstractObserver.prototype.completed=notImplemented;/**
   ...
   */var Observable=Rx.Observable=function(){function makeSubscribe(self,subscribe){return function(o){var oldOnError=o.onError;o.onError=function(e){makeStackTraceLong(e,self);oldOnError.call(o,e);};return subscribe.call(self,o);};}function Observable(){if(Rx.config.longStackSupport&&hasStacks){var oldSubscribe=this._subscribe;var e=tryCatch(thrower)(new Error()).e;this.stack=e.stack.substring(e.stack.indexOf('\n')+1);this._subscribe=makeSubscribe(this,oldSubscribe);}}observableProto=Observable.prototype;/**
    ...
    */Observable.isObservable=function(o){return o&&isFunction(o.subscribe);};/**
     ...
     */observableProto.subscribe=observableProto.forEach=function(oOrOnNext,onError,onCompleted){return this._subscribe((typeof oOrOnNext==='undefined'?'undefined':(0,_typeof3.default)(oOrOnNext))==='object'?oOrOnNext:observerCreate(oOrOnNext,onError,onCompleted));};/**
...
function fixSubscriber(subscriber){return subscriber&&isFunction(subscriber.dispose)?subscriber:isFunction(subscriber)?disposableCreate(subscriber):disposableEmpty;}function setDisposable(s,state){var ado=state[0],self=state[1];var sub=tryCatch(self.__subscribe).call(self,ado);if(sub===errorObj&&!ado.fail(errorObj.e)){thrower(errorObj.e);}ado.setDisposable(fixSubscriber(sub));}function AnonymousObservable(subscribe,parent){this.source=parent;this.__subscribe=subscribe;__super__.call(this);}AnonymousObservable.prototype._subscribe=function(o){var ado=new AutoDetachObserver(o),state=[ado,this];if(currentThreadScheduler.scheduleRequired()){currentThreadScheduler.schedule(state,setDisposable);}else{setDisposable(null,state);}return ado;};return AnonymousObservable;}(Observable);var AutoDetachObserver=function(__super__){inherits(AutoDetachObserver,__super__);function AutoDetachObserver(observer){__super__.call(this);this.observer=observer;this.m=new SingleAssignmentDisposable();}var AutoDetachObserverPrototype=AutoDetachObserver.prototype;AutoDetachObserverPrototype.next=function(value){var result=tryCatch(this.observer.onNext).call(this.observer,value);if(result===errorObj){this.dispose();thrower(result.e);}};AutoDetachObserverPrototype.error=function(err){var result=tryCatch(this.observer.onError).call(this.observer,err);this.dispose();result===errorObj&&thrower(result.e);};AutoDetachObserverPrototype.completed=function(){var result=tryCatch(this.observer.onCompleted).call(this.observer);this.dispose();result===errorObj&&thrower(result.e);};AutoDetachObserverPrototype.setDisposable=function(value){this.m.setDisposable(value);};AutoDetachObserverPrototype.getDisposable=function(){return this.m.getDisposable();};AutoDetachObserverPrototype.dispose=function(){__super__.prototype.dispose.call(this);this.m.dispose();};return AutoDetachObserver;}(AbstractObserver);var UnderlyingObservable=function(__super__){inherits(UnderlyingObservable,__super__);function UnderlyingObservable(m,u){this._m=m;this._u=u;__super__.call(this);}UnderlyingObservable.prototype.subscribeCore=function(o){return new BinaryDisposable(this._m.getDisposable(),this._u.subscribe(o));};return UnderlyingObservable;}(ObservableBase);var GroupedObservable=function(__super__){inherits(GroupedObservable,__super__);function GroupedObservable(key,underlyingObservable,mergedDisposable){__super__.call(this);this.key=key;this.underlyingObservable=!mergedDisposable?underlyingObservable:new UnderlyingObservable(mergedDisposable,underlyingObservable);}GroupedObservable.prototype._subscribe=function(o){return this.underlyingObservable.subscribe(o);};return GroupedObservable;}(Observable);/**
  ...
  */Rx.Pauser=function(__super__){inherits(Pauser,__super__);function Pauser(){__super__.call(this);}/**
    ...
    */Pauser.prototype.resume=function(){this.onNext(true);};return Pauser;}(Subject);if(typeof define=='function'&&(0,_typeof3.default)(define.amd)=='object'&&define.amd){root.Rx=Rx;define(function(){return Rx;});}else if(freeExports&&freeModule){// in Node.js or RingoJS
...
var rEndingLine=captureLine();}).call(this);});var Rx=interopDefault(rx_all);function Angle(degree){var _this=this;var normalize=function normalize(degree){//角度を0<=angle<360に正規化
if(degree<0){return normalize(degree+360);}if(degree>=360){return normalize(degree-360);}return degree;};this.degree=normalize(degree);this.abs=function(){return Math.abs(_this.degree180());};this.degree180=function(){if(_this.degree>180){return 360-_this.degree;}else{return _this.degree;}};this.angleBlock=function(){if(0<=_this.degree&&_this.degree<90){return"UpperRight";}else if(90<=_this.degree&&_this.degree<180){return"UpperLeft";}else if(180<=_this.degree&&_this.degree<270){return"LowerLeft";}else{return"LowerRight";}};this.add=function(other){return Angle.prototype.add(_this,other);};this.sub=function(other){return Angle.prototype.sub(_this,other);};}Angle.prototype.add=function(a,b){return new Angle(a.degree+b.degree);};Angle.prototype.sub=function(a,b){return new Angle(a.degree-b.degree);};var SwipeDirection={Right:"right",Left:"left",Up:"up",Down:"down"};function Swipe(startAnglePosition,endAnglePosition,timeDuration){this.getSpeed=function(){return Math.abs(endAnglePosition.sub(startAnglePosition).abs())/timeDuration*1000;};this.getDirection=function(){var s=startAnglePosition.angleBlock;var e=endAnglePosition.angleBlock;switch(s){case"UpperLeft":if(e=="UpperRight"){return SwipeDirection.Right;}else if(e=="LowerLeft"){return SwipeDirection.Down;}break;case"UpperRight":if(e=="UpperLeft"){return SwipeDirection.Left;}else if(e=="LowerRight"){return SwipeDirection.Down;}break;case"LowerLeft":if(e=="UpperLeft"){return SwipeDirection.Up;}else if(e=="LowerRight"){return SwipeDirection.Right;}break;case"LowerRight":if(e=="UpperRight"){return SwipeDirection.Up;}else if(e=="LowerLeft"){return SwipeDirection.Left;}break;}return startAnglePosition>endAnglePosition?SwipeDirection.Right:SwipeDirection.Left;};}var Util={test1:function test1(){console.log("hello!!");},average:function average(list){return list.reduce(function(sum,element){return sum+element;})/list.length;},takeLast:function takeLast(list,count){var clone=list.concat();if(list.length<=count){return clone;}clone.splice(0,list.length-count);return clone;},remove:function remove(list,item){for(var i=0;i<list.length;i++){if(list[i]===item){list.splice(i,1);return;}}},max:function max(list){return list.reduce(function(max,current){return max>current?max:current;});},min:function min(list){return list.reduce(function(min,current){return min<current?min:current;});}};function AngleFactory(leftBounds,rightBounds){var _this2=this;this.leftBounds=leftBounds;this.rightBounds=rightBounds;this.toAngle=function(pos){if(pos<_this2.leftBounds){pos=_this2.leftBounds;}else if(_this2.rightBounds<pos){pos=_this2.rightBounds;}var dir=pos-_this2.leftBounds;var limit=_this2.rightBounds-_this2.leftBounds;var rate=dir/limit;var correction=170;var angle=rate*360+correction;return new Angle(angle);};this.toAngleDelta=function(delta){var limit=_this2.rightBounds-_this2.leftBounds;var rate=delta/limit;var angle=rate*360;return new Angle(angle);};}Rx.Observable.prototype.list=function(){return this.scan(function(acc,current){var clone=acc.concat();if(clone.length>15){clone.splice(0,1);}clone.push(current);return clone;},[]);};Rx.Observable.prototype.pre=function(){return this.scan(function(acc,current){return[acc[1],current];},[null,null]);};var _ua=function(u){return{Tablet:u.indexOf("windows")!=-1&&u.indexOf("touch")!=-1&&u.indexOf("tablet pc")==-1||u.indexOf("ipad")!=-1||u.indexOf("android")!=-1&&u.indexOf("mobile")==-1||u.indexOf("firefox")!=-1&&u.indexOf("tablet")!=-1||u.indexOf("kindle")!=-1||u.indexOf("silk")!=-1||u.indexOf("playbook")!=-1,Mobile:u.indexOf("windows")!=-1&&u.indexOf("phone")!=-1||u.indexOf("iphone")!=-1||u.indexOf("ipod")!=-1||u.indexOf("android")!=-1&&u.indexOf("mobile")!=-1||u.indexOf("firefox")!=-1&&u.indexOf("mobile")!=-1||u.indexOf("blackberry")!=-1};}(window.navigator.userAgent.toLowerCase());function MBT(option,target){var _this3=this;this.option=option||{};this.target=target;this.setupStageLimit=this.option.setupStageLimit||10;//終了回数
...
var _eventListeners={};// eventName:callback array dictionary.
this.addListener=function(eventName,callback){if(!_eventListeners[eventName]){_eventListeners[eventName]=[];}_eventListeners[eventName].push(callback);};this.removeListener=function(eventName,callback){if(!_eventListeners[eventName]){return;}_eventListeners[eventName].remove(callback);if(_eventListeners[eventName].length===0){delete _eventListeners[eventName];}};var invoke=function invoke(eventName,args){var listeners=_eventListeners[eventName];if(listeners){listeners.forEach(function(listener){listener.apply(_this3,args);});}};// ------- registration --------
var onTapSubject=new Rx.Subject();var onDoubleTapSubject=new Rx.Subject();var onScrollBeganSubject=new Rx.Subject();var onScrollEndedSubject=new Rx.Subject();var onScrollSubject=new Rx.Subject();var onSwipeSubject=new Rx.Subject();var onSetupProgressSubject=new Rx.Subject();var onSetupCompletedSubject=new Rx.Subject();var touchBeganSubject=new Rx.Subject();var touchMovedSubject=new Rx.Subject();var touchEndedSubject=new Rx.Subject();onSetupProgressSubject.subscribe(function(){invoke("setupprogress");});onSetupCompletedSubject.subscribe(function(){invoke("setupcompleted");});onTapSubject.subscribe(function(){invoke("tap");});onDoubleTapSubject.subscribe(function(){invoke("doubletap");});onScrollBeganSubject.subscribe(function(){invoke("scrollbegan");});onScrollSubject.subscribe(function(delta){invoke("scroll",[delta]);});onScrollEndedSubject.subscribe(function(){invoke("scrollended");});onSwipeSubject.subscribe(function(swipe){invoke("swipe",[swipe.speed,swipe.direction]);});var subscriptions=[];this.reset=function(){subscriptions.forEach(function(subscription){subscription.unsubscribe();});subscriptions.length=0;};/**
     * セットアップの開始
     * @param  {[type]} doneCallback [description]
     * @return {[type]}              [description]
     */this.setup=function(){var _this4=this;//setup subscribe
console.log("setupStart");this.reset();var angleFactory=new AngleFactory(500,590);var upOrDown=Rx.Observable.merge(touchBeganSubject,touchEndedSubject);var moveOverBounds=touchMovedSubject.bufferWithCount(2,1).filter(function(val,index){return Math.abs(val[0]-val[1])>100;}).flatMap(function(it){return Rx.Observable.of(it[0],it[1]);});var setupSignal=Rx.Observable.merge(upOrDown,moveOverBounds);var average=setupSignal.list().map(function(list){return Util.average(list);});var left=Rx.Observable.zip(setupSignal,average).filter(function(it){return it[0]<it[1];}).map(function(it){return it[0];});var right=Rx.Observable.zip(setupSignal,average).filter(function(it){return it[0]>=it[1];}).map(function(it){return it[0];});var leftFin=left.list().map(function(it){return Util.takeLast(it,10);}).doOnNext(function(x){onSetupProgressSubject.onNext(null);}).filter(function(it){return it.length==10;}).filter(function(it){var clone=it.concat();var max=Util.max(clone);var min=Util.min(clone);Util.remove(clone,max);Util.remove(clone,min);var diff=max-min;console.log("left diff: "+diff);return diff<50;});var rightFin=right.list().map(function(it){return Util.takeLast(it,10);}).doOnNext(function(x){onSetupProgressSubject.onNext(null);}).filter(function(it){return it.length==10;}).filter(function(it){var clone=it.concat();var max=Util.max(clone);var min=Util.min(clone);Util.remove(clone,max);Util.remove(clone,min);var diff=max-min;console.log("right diff: "+diff);return diff<50;});subscriptions.push(Rx.Observable.combineLatest(leftFin.timestamp(),rightFin.timestamp()).filter(function(it){return Math.abs(it[0].timestamp-it[1].timestamp)<5000;}).first().subscribe(function(it){angleFactory.leftBounds=Util.average(it[0].value);angleFactory.rightBounds=Util.average(it[1].value);// console.log("aaa", "complete!! left:" + angleFactory.leftBounds + " right: " + angleFactory.rightBounds);
onSetupCompletedSubject.onNext(null);}));var detectAngles=Rx.Observable.merge(touchBeganSubject.map(function(it){return["began",it];}),touchMovedSubject.map(function(it){return["moved",it];}),touchEndedSubject.map(function(it){return["ended",it];})).skipUntil(onSetupCompletedSubject).map(function(it){return[it[0],angleFactory.toAngle(it[1])];}).publish();var sequence=detectAngles.skipWhile(function(it){return it[0]!=="began";}).takeUntil(detectAngles.debounce(50).filter(function(it){return it[0]==="ended";}));var seq_began=sequence.map(function(it){return it[1];}).filter(function(x,idx){return idx===0;}).repeat().publish();var seq_move=sequence.skip(1).map(function(it){return it[1];}).repeat().publish();var seq_ended=sequence.last({defaultValue:null}).filter(function(x){return x;}).map(function(it){return it[1];}).repeat().publish();seq_began.connect();seq_move.connect();seq_ended.connect();detectAngles.connect();var tap=sequence.first({defaultValue:null}).filter(function(x){return x;}).timestamp().zip(sequence.last({defaultValue:null}).timestamp()).repeat().filter(function(it){return it[1].timestamp-it[0].timestamp<_this4.tapDetectDuration;})//時間制限
.filter(function(it){return it[1].value[1].sub(it[0].value[1]).abs()<_this4.tapDetectTolerance;})//移動距離制限
.map(function(it){return it[1];});//publish?
subscriptions.push(tap.pre().map(function(it){var preTime=it[0]?it[0].timestamp:0;var cond=it[1].timestamp-preTime<500;return cond?"doubleTap":"tap";}).subscribe(function(it){if(it==="tap"){onTapSubject.onNext(null);}else{onDoubleTapSubject.onNext(null);}}));var moveDelta=seq_move.pre().map(function(it){return[it[1],it[1].sub(it[0]||it[1])];});//move,moveDelta
var scrollStart=moveDelta.map(function(it){return it[1].degree180();}).filter(function(deltaScale){return 0.3<Math.abs(deltaScale)&&Math.abs(deltaScale)<30;});var scrollStroke=scrollStart.takeUntil(seq_ended);subscriptions.push(scrollStroke.take(1).doOnNext(function(it){onScrollBeganSubject.onNext(null);}).zip(scrollStroke.count()).repeat().subscribe());subscriptions.push(scrollStroke.count().repeat().subscribe(function(it){if(it!==0){onScrollEndedSubject.onNext(null);}}));subscriptions.push(scrollStroke.repeat().subscribe(function(delta){onScrollSubject.onNext(delta);}));subscriptions.push(sequence.timestamp().toArray().repeat().filter(function(it){return it.length>=2;}).subscribe(function(it){var startAngle=it[0].value[1];var endAngle=it[it.length-1].value[1];var angleDelta=startAngle.sub(endAngle).abs();var timeDelta=it[it.length-1].timestamp-it[0].timestamp;var positionCondition=angleDelta>_this4.tapDetectTolerance;var timeCondition=timeDelta<_this4.tapDetectDuration;if(positionCondition&&timeCondition){//swipe
var swipe=new Swipe(startAngle,endAngle,timeDelta);onSwipeSubject.onNext({speed:swipe.getSpeed(),direction:swipe.getDirection()});}}));};this.test=function(arg){invoke("test",[arg]);};//detection click event
var tapFlag=false;if(_ua.Mobile){console.log("agent is mobile");this.target.addEventListener("touchstart",function(e){// var x = e.pageX ;
var x=e.changedTouches[0].pageX;tapFlag=true;//_state.mousedown(x);
touchBeganSubject.onNext(x);});this.target.addEventListener("touchmove",function(e){if(!tapFlag){return;}var x=e.changedTouches[0].pageX;//_state.mousemove(x);
touchMovedSubject.onNext(x);});this.target.addEventListener("touchend",function(e){//_state.mouseup(e.changedTouches[0].pageX);
touchEndedSubject.onNext(e.changedTouches[0].pageX);tapFlag=false;});}else{console.log("agent is not mobile");this.target.addEventListener("mousedown",function(e){tapFlag=true;var x=e.pageX;//_state.mousedown(x);
touchBeganSubject.onNext(x);});this.target.addEventListener("mouseup",function(e){tapFlag=false;//_state.mouseup(e.pageX);
touchEndedSubject.onNext(e.pageX);});this.target.addEventListener("mousemove",function(e){if(!tapFlag){return;}touchMovedSubject.onNext(e.pageX);//_state.mousemove(e.pageX);
});}}window.MilboxTouch=MBT;

}).call(this,require('_process'),typeof global !== "undefined" ? global : typeof self !== "undefined" ? self : typeof window !== "undefined" ? window : {})
},{"_process":77,"babel-runtime/core-js/object/create":2,"babel-runtime/core-js/object/keys":3,"babel-runtime/core-js/symbol":4,"babel-runtime/core-js/symbol/iterator":5,"babel-runtime/helpers/typeof":6}],2:[function(require,module,exports){
...
},{"../../modules/_core":16,"../../modules/es6.object.create":69}],8:[function(require,module,exports){
...
},{"./_export":23,"./_object-create":40}],70:[function(require,module,exports){
